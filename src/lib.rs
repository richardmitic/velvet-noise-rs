...
/// Iterator that provides just the impluse locations of a velvet noise sequence.
/// This should be the most efficient thing to calculate
pub struct VelvetImpulseLocations {
    m: std::ops::RangeFrom<usize>,
    td: usize,
    r1m: SmallRng
}

impl VelvetImpulseLocations {
    /// density is non-zero pulses per second
    /// sample_rate is total samples per second
    pub fn new(density: usize, sample_rate: usize) -> VelvetImpulseLocations {
        VelvetImpulseLocations {
            m: (0..),
...
impl Iterator for VelvetImpulseLocations {
    type Item = usize;
...
pub struct ChunkedVelvetImpulseLocations {
    impulses: VelvetImpulseLocations,
    chunk_length: usize,
    base: usize,
    store: Option<usize>
}

impl ChunkedVelvetImpulseLocations {
    pub fn new(density: usize, sample_rate: usize, chunk_length: usize) -> ChunkedVelvetImpulseLocations {
        ChunkedVelvetImpulseLocations {
            impulses: VelvetImpulseLocations::new(density, sample_rate),
...
impl Iterator for ChunkedVelvetImpulseLocations {
    type Item = Vec<usize>;
...
impl VelvetNoise {
    fn new(density: usize, sample_rate: usize) -> VelvetNoise {
        let mut imps = VelvetImpulseLocations::new(density, sample_rate);
...
mod tests {
    ...
    #[test]
    fn window_size() {
        let vil = VelvetImpulseLocations::new(441, 44100);
    ...
    fn iter_locations() {
        ...
        let vil = VelvetImpulseLocations::new(density, sample_rate);
    ...
    fn iter_chunked_locations() {
        ...
        let cvil = ChunkedVelvetImpulseLocations::new(density, sample_rate, chunk_size);
    ...
    fn single_chunk() {
        ...
        let chunk = ChunkedVelvetImpulseLocations::new(density, sample_rate, 960).next().unwrap();
        println!("{:?}", chunk);
    }
